...
$toolsDirPath = Split-Path -Parent $helpersPath
...
#>
  param(
    [Alias("LogPath")][parameter(Mandatory = $false, Position = 1)][string] $Path = $(Join-Path "$packagePath" -ChildPath "shortcuts.txt"),
